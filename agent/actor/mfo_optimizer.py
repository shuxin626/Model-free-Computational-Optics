...
class MFOOptimizer(Optimizer):
    ...
    def sampled_mask_preprocessing(self, sampled_masks):
        sampled_masks = sampled_masks[:, None, ...]
        if self.actor_param['output_normalize_flag']:
            sampled_masks = self.output_normalize(sampled_masks)
        else:
            sampled_masks = sampled_masks * math.pi
        phase_masks = sampled_masks.clone().permute(1, 0, *range(2, sampled_masks.dim()))
        return phase_masks, sampled_masks
    ...
    def step(self, rewards):
        for groups in self.param_groups:
            ...
            slm_mask = groups['params'][0]
            slm_mask_clone = groups['params'][1]
            # - slm_mask.data [1, qb, h, w] or [1, qb, h, w, layers]
            # - cam_image [b, qb, h, w]
            # - rewards [qb]
